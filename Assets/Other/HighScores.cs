...
{
    const string privateCode = "AbGpW8ixGUqu4PzvuMiLugTtphN-VTtEqByoaegyV3Yg";  //Key to Upload New Info
    const string publicCode = "63a2420c8f40bb1070d0d4f1";   //Key to download
    ...
    IEnumerator DatabaseDownload()
    {
        ...
        if (string.IsNullOrEmpty(www.downloadHandler.error))
        {
            ...
            Debug.Log("sl"+scoreList);
            OrganizeInfo(www.downloadHandler.text);
            myDisplay.SetScoresToMenu(scoreList);
        }
        else print("Error uploading" + www.downloadHandler.error);
    }
